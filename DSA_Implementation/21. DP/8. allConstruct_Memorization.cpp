...
///Write a function 'allConstruct(target, string)' that accepts a target
///string and an array of strings

///The function should return a 2D array containing all the number of ways that the target 
///can be constructed by concatenating elements of the string array.
///Each element of 2D array should represent one combination that construct
///the target. 
...
//Time: O(n^m) ==>m = targetSum, n = nums.size() 
//Space: O(n^m)
int allConstructWithTabulation(vector<string> &str, string target){
	vector<vector<int>> dp(target.size()+1, vector<int> (target.size()+1));
	dp[0] = [[1]];
	...
	for(int i = 0;i<=target.size();i++){
		for(int j=0;j<str.size();j++){				
			if(target.substr(i, i+str[j].size())==str[j]){ //compare prefix
				...
				const newComninations = dp[i].map(subArray => [...subArray, nums[j] ]
				dp[i+str[j].size()].push(newCombinations);
			}
		}
	}
