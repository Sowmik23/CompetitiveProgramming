...
/// * It generates the primes below 10^10 int 7.25 sec
/// * on an Intel Core i7-6700 3.4 GHz CPU.
/// * 
/// * Where I learnt it: https://primesieve.org/segmented_bit_sieve.cpp
/// */ 
...
/// @param limt Sieve primes <= limit.
void segmented_sieve(int64_t limit){
	int64_t sqrtt = (int64_t) sqrt(limit);
	int64_t sieve_size = max(sqrtt, L1D_CACHE_SIZE);
	...
	vector<uint8_t> sieve(sieve_size);
	vector<uint8_t> is_prime(sqrtt + 1, true);
...
int main(int argc, char** argv) {
	...
	if(argc>=2) {
		...
		segmented_sieve(1018650200);  ///10^9
	}
